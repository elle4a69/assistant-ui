...
try:
    ...
    from backend.core.state import _agent_run_tasks
    ...
    from core.database import get_db
    from core.clients import openai_client
    from core.state import _agent_run_tasks
    from models.domain import (
        OperationsAgentRun, OperationsAgentEvent, OperationsChatMessage, OperationsMemory,
        Thread, Message, ThreadEvent
    )
...
# ==============================================================================
# 3. ASSEMBLE SLIMMED BACKEND/MAIN.PY
# ==============================================================================
prefix = MAIN_LINES[:1427]

middle_blocks = [
    "\n# Re-export all domain services and route handlers for 100% backward compatibility\n",
    ...
    "# Mount Domain Routers in exact order\n",
...
suffix = MAIN_LINES[7202:]

new_main_content = "".join(prefix) + "".join(middle_blocks) + "".join(suffix)

with open(BACKEND_DIR / "main.py", "w", encoding="utf-8") as f:
    f.write(new_main_content)
print(f"Updated backend/main.py (lines: {len(new_main_content.splitlines())})")
