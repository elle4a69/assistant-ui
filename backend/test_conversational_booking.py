...
def test_booking_requires_notice_allows_adjacency_and_rejects_overlaps(monkeypatch):
    ...
    calendar.busy = [{"start": candidate - timedelta(minutes=30), "end": candidate}]
    assert booking_availability_error(candidate, 30) is None
    calendar.busy = [{"start": candidate + timedelta(minutes=30), "end": candidate + timedelta(minutes=60)}]
    assert booking_availability_error(candidate, 30) is None
    calendar.busy = [{"start": candidate - timedelta(minutes=1), "end": candidate + timedelta(minutes=10)}]
    assert booking_availability_error(candidate, 30) == "That time overlaps another booking."
    calendar.busy = [{"start": candidate + timedelta(minutes=29), "end": candidate + timedelta(minutes=60)}]
    assert booking_availability_error(candidate, 30) == "That time overlaps another booking."
