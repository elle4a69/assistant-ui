...
export default function SettingsView() {
  ...
  // Add-ons are shared by both lines and persisted outside the service lists.
  ...
  const handleAddServiceAddon = (event: React.FormEvent) => {
    ...
    triggerBanner('success', 'Add-on added. Save add-ons to apply.');
  };
  ...
  const handleUpdateServiceAddon = (event: React.FormEvent) => {
    ...
    setServiceAddons(current => current.map(addon => addon.id === editingAddonId ? {
      ...addon,
    ...
    triggerBanner('success', 'Add-on updated. Save add-ons to apply.');
  };
  ...
  const handleSaveServiceAddons = async () => {
    ...
    try {
      ...
      triggerBanner('success', 'Shared service add-ons saved.');
    } catch (err) {
      console.error(err);
      triggerBanner('error', 'Failed to save service add-ons.');
    } finally {
            ...
            <details name="settings-sections" className="settings-section bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
              ...
              <div className="p-3 sm:p-5 flex flex-col gap-6 font-sans">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <section aria-labelledby="service-addons-heading" className="flex flex-col gap-3 rounded-xl border border-emerald-200 bg-emerald-50/40 p-3.5">
                    <div>
                      <h3 id="service-addons-heading" className="text-xs font-bold text-slate-800">Service Add-ons &amp; Extras</h3>
                      <p className="mt-0.5 text-[10px] text-slate-600">
                        Shared across Line 1 and Line 2 for the AI to mention with services. Add-ons are informational only and cannot be booked as standalone services.
                      </p>
                    </div>
                    ...
                    <div className="overflow-hidden rounded-lg border border-emerald-200 bg-white divide-y divide-slate-200">
                      ...
                      ) : serviceAddons.map(addon => editingAddonId === addon.id ? (
                        <form key={addon.id} onSubmit={handleUpdateServiceAddon} className="flex flex-col gap-2.5 bg-emerald-50/50 p-3">
                          <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
                            ...
                            <input required type="number" min="0" value={editAddonPrice} onChange={event => setEditAddonPrice(Number(event.target.value) || 0)} aria-label="Extra price" className="rounded border border-slate-300 bg-white p-1.5 text-xs" />
                            <input required type="number" min="0" value={editAddonDuration} onChange={event => setEditAddonDuration(Number(event.target.value) || 0)} aria-label="Extra time in minutes" className="rounded border border-slate-300 bg-white p-1.5 text-xs" />
                          </div>
                          <textarea value={editAddonDesc} onChange={event => setEditAddonDesc(event.target.value)} placeholder="Description" rows={2} className="rounded border border-slate-300 bg-white p-1.5 text-xs" />
                        ...
                        <div key={addon.id} className="flex items-start justify-between gap-3 p-3">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              <span className="rounded border border-emerald-200 bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-800">shared · non-bookable</span>
                            </div>
                          ...
                          <div className="flex shrink-0 items-center gap-2 text-xs font-bold text-slate-700">
                            ...
                            <button type="button" onClick={() => startEditServiceAddon(addon)} title="Edit add-on" className="rounded p-1 text-indigo-600 hover:bg-indigo-50"><Edit className="h-3.5 w-3.5" /></button>
                            <button type="button" onClick={() => setServiceAddons(current => current.filter(item => item.id !== addon.id))} title="Delete add-on" className="rounded p-1 text-rose-600 hover:bg-rose-50"><Trash2 className="h-3.5 w-3.5" /></button>
                          </div>
                        </div>
                      ))}
                    </div>

                    <form onSubmit={handleAddServiceAddon} className="flex flex-col gap-2.5 rounded-lg border border-emerald-200 bg-white p-3">
                      <h4 className="flex items-center gap-1 text-xs font-bold text-slate-700"><Plus className="h-3.5 w-3.5" /> Add new add-on</h4>
                      <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
                        <input required value={newAddonName} onChange={event => setNewAddonName(event.target.value)} placeholder="Name" className="rounded border border-slate-300 p-2 text-xs md:col-span-2" />
                        <input required type="number" min="0" value={newAddonPrice} onChange={event => setNewAddonPrice(Number(event.target.value) || 0)} placeholder="Extra price ($)" aria-label="New add-on extra price" className="rounded border border-slate-300 p-2 text-xs" />
                        <input required type="number" min="0" value={newAddonDuration} onChange={event => setNewAddonDuration(Number(event.target.value) || 0)} placeholder="Extra time (minutes)" aria-label="New add-on extra time in minutes" className="rounded border border-slate-300 p-2 text-xs" />
                      </div>
                      <textarea value={newAddonDesc} onChange={event => setNewAddonDesc(event.target.value)} placeholder="Description" rows={2} className="rounded border border-slate-300 p-2 text-xs" />
                      <button type="submit" className="self-end rounded bg-emerald-700 px-3.5 py-1.5 text-[11px] font-bold text-white hover:bg-emerald-800">Add to list</button>
                    </form>

                    <button type="button" onClick={handleSaveServiceAddons} disabled={savingServiceAddons} className="self-end rounded-lg bg-emerald-700 px-4 py-2.5 text-xs font-bold text-white hover:bg-emerald-800 disabled:opacity-50">
                      {savingServiceAddons ? 'Saving add-ons...' : 'Save Service Add-ons'}
                    </button>
                  </section>
